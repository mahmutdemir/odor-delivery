function ControlParadigm = make_ctrl_prdgms_044_WA_pulses(vol,pulsevol,initonoff,inittime,width,interval,n_of_pulses,T,save_name,saveit,main_air)
%  ControlParadigm = make_ctrl_prdgms_044_WA_pulses(vol,pulsevol,initonoff,inittime,width,interval,n_of_pulses,T,save_name,saveit,main_air)
% function ControlParadigm = make_ctrl_prdgms_044_WA_pulses(vol,pulsevol,initonoff,inittime,width,interval,n_of_pulses,T,save_name,saveit,main_air)
...
if T==0
    for wn = 1:length(width)
        ...
        for cn = 1:length(odorvol)
            ...
            voltages(2,:) = odorvol(cn)/200*5;  % write voltage for the MFC
        ...
        for cn = 1:length(odorvol)
            ...
            voltages(2,inittime*sr+1:end-1) = pulsevol/200*5;  % write voltage for the MFC
            ControlParadigm(parad_num).Outputs = voltages;
