function ControlParadigm = make_ctrl_prdgms_021(osc_freq,corr_length,gate_corr,inittime,width,duration,save_name,saveit)
%  ControlParadigm = make_ctrl_prdgms_021(corr_length,gate_corr,inittime,width,duration,save_name,saveit)
% function ControlParadigm = make_ctrl_prdgms_021(corr_length,gate_corr,inittime,width,duration,save_name,saveit)
...
%%   Inputs
%
%   osc_freq: oscillation frequency for the MFC's
...
max_rate = 30;  % the max flow rate to be 300 ml/min
    ...
    odor_rate = max_rate*sin(2*pi*osc_freq*time);
    mix_rate = -odor_rate;
