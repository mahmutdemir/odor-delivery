function ControlParadigm = make_ctrl_prdgms_037(odor_vol,mix_vol,bckg_vol,bmix_vol,inittime,width,lag,interval,nofpulses,save_name,saveit)
%  ControlParadigm = make_ctrl_prdgms_037(odor_vol,mix_vol,bckg_vol,bmix_vol,inittime,width,lag,interval,nofpulses,save_name,saveit)
% function ControlParadigm = make_ctrl_prdgms_037(odor_vol,mix_vol,bckg_vol,bmix_vol,inittime,width,lag,interval,nofpulses,save_name,saveit)
...
% modified on 10.31.14 after the delivery system is changed
...
%   bmix_vol:   (vector) flow rate passing through the bck dilution bottle
...
sr = 10000;  % digitization sampling rate Hz 
    ...
    % 1: odor_mix, 2: bck_dil, 3: odor, 4: Background 5: odor_puff, 6: bckg_puff
    % 7: Main
    voltages = zeros(7,10000); % allocate space for the output matrix and set to zero
    voltages(7,:) = 1;  % write voltage for the main air MFC
                ...
                for pn = 1:length(bckg_vol)
                    ...
                    if ~(odor_vol(cn)==0 && bckg_vol(pn)==0)
                        if odor_vol(cn)==0 && pflag(pn,wn) ==0
                            ...
                            voltages = zeros(7,nop); % allocate space for the output matrix and set to zero
                            ...
                            bmix_volt = bmix_vol(pn)/500*5;   % voltage for air mix MFC (Alicat 500ml)
                % 1: odor_mix, 2: bck_dil, 3: odor, 4: Background 5: odor_puff, 6: bckg_puff,  7: Main
                            voltages(7,:) = 1;  % write voltage for the main air MFC
                            voltages(2,1:end-1) = bmix_volt;  % write voltage for the odor MFC
                            voltages(3,1:end-1) = odor_volt;  % write voltage for the odor MFC
                            voltages(1,1:end-1) = mix_volt;  % write voltage for the mix air MFC
                            voltages(4,1:end-1) = bckg_volt;  % write voltage for the mix air MFC


                            for j = 1: nofpulses
                                if odor_vol(cn)~=0
                                    voltages(6,round((inittime+(j-1)*(interval+lag(i)))*sr)+1:...
                                        round((inittime+width(wn)+(j-1)*(interval+lag(i)))*sr)+1) = 1;  % write voltage for the puff valve
                                end
                                if bckg_vol(pn)~=0
                                    voltages(5,round((inittime+(j-1)*(interval+lag(i))+width(wn)+lag(i))*sr)+1:...
                            ...
                            ControlParadigm(parad_num).Name = ['c_' num2str(odor_vol(cn)) '_' num2str(mix_vol(cn)) ...
                                '_p_' num2str(bckg_vol(pn)) '_' num2str(bmix_vol(pn)) '_l_' num2str(lag(i)*1000) '_w_' num2str(width(wn)*1000)];
